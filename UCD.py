...
from keras.layers import Dense, Concatenate, BatchNormalization, Activation, Input
...
auc_ls=[]
for t in range(10):
    with open('newdata.pickle', 'rb') as handle:
    ...
    g = nx.Graph()
    edgelist = pd.read_csv('source_target_new.csv')
    for i, elrow in edgelist.iterrows():
        g.add_edge(elrow[0].strip('\n'), elrow[1].strip('\n'))
    adj = nx.adjacency_matrix(g)
    user_attributes = pd.read_csv('user_attribute_new.csv')
    ...
    pre_ls.append(precision_recall_fscore_support(y_single, label_pred)[0][1])
    rec_ls.append(precision_recall_fscore_support(y_single, label_pred)[1][1])
    f1_ls.append(precision_recall_fscore_support(y_single, label_pred)[2][1])
    auc_ls.append(roc_auc_score(y_single, label_pred))

print(np.mean(pre_ls), np.std(pre_ls))
print(np.mean(rec_ls), np.std(rec_ls))
print(np.mean(f1_ls), np.std(f1_ls))
